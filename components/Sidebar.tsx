...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, userName, onLogout }) => {
  const menuItems = [
    { id: AppView.CHAT, label: 'Pro 지능형 채팅', icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z' },
  ...
  return (
    <aside className="w-64 glass-morphism h-full flex flex-col border-r border-white/5 hidden md:flex">
      ...
      <nav className="flex-1 px-4 space-y-1">
        ...
        <div className="mt-8 px-4 py-6 rounded-2xl bg-gradient-to-br from-indigo-900/40 to-slate-900/40 border border-white/5 text-center">
           ...
           <p className="text-xs font-bold text-slate-300">엔터프라이즈 보안 적용</p>
           <p className="text-[9px] text-slate-500 mt-1 uppercase font-black">All Features Unlocked</p>
        </div>
      </nav>
